...
/** Intermediate representation of the code. There are 5 broad categories of
  * code: `Type`, `Term`, `Defn`, `Decl`, and `Logic`.
  */
object IR {
  sealed trait IR
  ...
  object Type {
    ...
    final case class TypeParameter(
        ...
        upperTypeBound: Option[Type]
    ) extends Type // e.g. `A` in `Foo[A[_, _]]`
  ...
  object Term {
    ...
    final case class Reference(name: String)
        extends Term // name that refers to a constant
    ...
    final case class UnaryOperation(op: Op.UnaryOperator, arg: Term)
        extends Term

    final case class InstantiateClass(
        clazz: String,
        tparams: List[Type.Type],
        args: List[Term]
    ) extends Term
    ...
    final case class MethodReference(
        ...
        method: String,
        signature: Type.Type
    ) extends Term // reference to a method, e.g. `this.plus`
  ...
  object Pat {
    ...
    final case class Extract(tpe: Type.Type, vars: List[VarOrWildcard])
        extends Pat
  }
  ...
  object Decl {
    ...
    final case class Method(
        name: String,
        mods: List[Mod.Mod],
        tparams: List[Type.TypeParameter],
        args: List[Defn.Arg],
        retType: Type.Type
    ) extends Decl
  }
  ...
  object Defn {
    ...
    final case class Arg(name: String, tpe: Type.Type)
        extends Defn // default value is not needed because omitted arguments are added by the compiler when doing the method call (or instantiating the class)
    final case class ValDef(name: String, tpe: Type.Type, value: Term.Term)
        extends Defn
        with TermOrValDef
        with Stat
    ...
    final case class CtxDef(
        method: String,
        tparams: List[Type.TypeParameter],
        args: List[Arg],
        body: Term.Term
    ) extends Predicate
    final case class PreDef(
        method: String,
        tparams: List[Type.TypeParameter],
        args: List[Arg],
        body: Term.Term
    ) extends Predicate
    final case class InvDef(
        method: String,
        tparams: List[Type.TypeParameter],
        args: List[Arg],
        body: Term.Term
    ...
    final case class Field(mods: List[Mod.Mod], name: String, tpe: Type.Type)
        extends Defn
    final case class ClassDef(
        annots: List[Mod.Annot],
        name: String,
        tparams: List[Type.TypeParameter],
        fields: List[Field],
        methods: List[MethodOrPredicate],
        inheritedMethods: List[MethodOrPredicate],
        supert: Option[Type.Complex]
    ) extends Defn
  ...
  object Logic {
    ...
    final case class Implication(antecedent: Term.Term, consequent: Term.Term)
        extends Logic
        with Term.Term
    final case class Forall(variables: Set[Variable], body: Term.Term)
        extends QuantifiedFormula
    final case class Exists(variables: Set[Variable], body: Term.Term)
        extends QuantifiedFormula
  }
}

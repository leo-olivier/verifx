...
object Z3CompilerPlugin {
  sealed trait Z3Exp {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp]
  }

  final case class ValDef(name: String, tpe: Z3Exp, value: Z3Exp)
      extends Z3Exp {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] =
  ...
  final case class Z3Wrapper(exp: String) extends Z3Exp {
    ...
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = State.pure(this)
  }
  ...
  implicit class FillableArg(arg: Plugin.Arg[Z3Exp]) {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Plugin.Arg[Z3Exp]] = {
      arg.tpe
        .fillTypeParameters(substitutions)
        .map(filledTpe => arg.copy(tpe = filledTpe))
    }
  }
  ...
  implicit class FillableField(field: Plugin.Field[Z3Exp]) {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Plugin.Field[Z3Exp]] = for {
  ...
  final case class MethodWithFullName(methodName: MethodName, method: Method)
      extends Z3Exp {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = for {
      filledMethodName <- methodName.fillTypeParameters(substitutions)
      filledMethod <- method.fillTypeParameters(substitutions)
    } yield copy(
      filledMethodName.asInstanceOf[MethodName],
      filledMethod.asInstanceOf[Method]
    )
  }
  ...
  final case class Method(
      ...
      name: String,
      tparams: List[Z3Type],
      args: List[Plugin.Arg[Z3Exp]],
      body: Z3Exp,
      retTpe: Z3Type,
      isRecursive: Boolean,
      context: Option[Ctx],
      precondition: Option[Pre],
      invariant: Option[Inv]
  ) extends MethodOrPredicate {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = for {
      filledTParams <- tparams.traverse(tparam =>
        tparam.fillTypeParameters(substitutions)
      )
      ...
      filledPrecondition <- precondition.traverse(
        _.fillTypeParameters(substitutions)
      )
      filledInvariant <- invariant.traverse(_.fillTypeParameters(substitutions))
    } yield copy(
      tparams = filledTParams.asInstanceOf[List[Z3Type]],
      args = filledArgs,
      body = filledBody,
      retTpe = filledRetTpe.asInstanceOf[Z3Type],
    ...
    def setFullName(className: String, classTParams: List[Z3Type]) =
      MethodWithFullName(
        MethodName(className, classTParams, name, tparams),
        this
      )
  }

  final case class MethodWithoutSpec(
      mods: List[Z3Exp],
      name: String,
      tparams: List[Z3Type],
      args: List[Plugin.Arg[Z3Exp]],
  ...
  final case class Ctx(name: String, args: List[Plugin.Arg[Z3Exp]], body: Z3Exp)
      extends Predicate {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = for {
  ...
  final case class Pre(name: String, args: List[Plugin.Arg[Z3Exp]], body: Z3Exp)
      extends Predicate {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = for {
  ...
  final case class Inv(name: String, args: List[Plugin.Arg[Z3Exp]], body: Z3Exp)
      extends Predicate {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = for {
  ...
  /** Represents a type in Z3. Enumerations are treated specially because their
    * data constructors are not types of their own in Z3. Hence, data
    * constructor type are compiled to the type of the enumeration. However, to
    * construct an instance of the enumeration we need the data constructor but
    * the type got compiled to the enumeration's type. Therefore, `Z3Type` also
    * keeps an `ogDataCtorTpe` which stores the "type" of the data constructor
    * that got replaced by the enumeration's type. If this type is not an
    * enumeration type, then `ogDataCtorTpe` is `None`.
    */
  final case class Z3Type(
      name: String,
      typeArgs: List[Z3Type],
      ogDataCtorTpe: Option[Z3Type] = None
  ) extends Z3Exp {
    ...
    def fillTypeParameters(
      ...
      substitutions.get(this) match {
        ...
        case None         => {
          for {
            filledTypeArgs <- typeArgs.traverse(
              _.fillTypeParameters(substitutions)
            )
  ...
  final case class BinaryOperation(operator: String, left: Z3Exp, right: Z3Exp)
      extends Z3Exp {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = for {
  ...
  final case class If(condition: Z3Exp, thenb: Z3Exp, elseb: Z3Exp)
      extends Z3Exp {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = for {
  ...
  final case class QualifiedIdentifier(identifier: Z3Exp, tpe: Z3Exp)
      extends Z3Exp {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = for {
  ...
  final case class MethodCall(
    ...
    def fillTypeParameters(
      ...
      filledClass <- classType
        .fillTypeParameters(substitutions)
        .asInstanceOf[State[CompilerState, Z3Type]]
      filledTArgs <- targs.traverse(targ =>
        targ
          .fillTypeParameters(substitutions)
          .asInstanceOf[State[CompilerState, Z3Type]]
      )
      _ <- concretizeClassIfNeeded(
        op.method,
        filledClass,
        filledTArgs
      ) // important to concretize the class type such that the method is created if it does not yet exist
      filledOp <- op.asInstanceOf[Z3Exp].fillTypeParameters(substitutions)
      filledArgs <- args.traverse(_.fillTypeParameters(substitutions))
    } yield copy(
      classType = filledClass,
      op = filledOp.asInstanceOf[MethodName],
      targs = filledTArgs,
      args = filledArgs
    )
  ...
  case class MethodName(
      className: String,
      classTypeParams: List[Z3Type],
      method: String,
      methodTypeParams: List[Z3Type]
  ) extends Z3Exp {
    def fillTypeParameters(
        substitutions: Map[Z3Type, Z3Type]
    ): State[CompilerState, Z3Exp] = {
      ...
      for {
        filledClazz <- clazz
          .fillTypeParameters(substitutions)
          .asInstanceOf[State[CompilerState, Z3Type]]
        filledMethodTypeParams <- methodTypeParams.traverse(tparam =>
          tparam
            .fillTypeParameters(substitutions)
            .asInstanceOf[State[CompilerState, Z3Type]]
        )
      } yield copy(
        className = filledClazz.asInstanceOf[Z3Type].name,
        classTypeParams = filledClazz.typeArgs,
        methodTypeParams = filledMethodTypeParams
      )
    }

    override def toString: Z3Code =
      makeFunName(className, classTypeParams, method, methodTypeParams)
  }

  sealed trait Case extends Z3Exp

  /** Case that matches ADT constructor. e.g.:
    * `case Insert(pos, char, _) => ...`
    * @param tpe
    *   Name of the constructor
    * @param vars
    *   Variables used in the pattern (in order)
    */
  ...
  object Implies {
    def apply(antecedent: Z3Exp, consequent: Z3Exp) =
  ...
  object Get {
    def apply(exp: Z3Exp, tpe: Z3Exp) =
      Apply(QualifiedIdentifier("get", tpe), List(exp))
  }
  ...
  private def isConcreteType(
    ...
    exp match {
      case Z3Type(className, typeArgs, _) => {
        isPrimitive(exp) ||
        (classes.contains(className) && typeArgs.forall(
          isConcreteType(_, classTable)
        )) ||
        (className.contains("<") && className.contains(
          ">"
        )) // this is a type parameter that we declared, e.g. for a class Foo[V] we declare a new sort Foo<V> so Foo<V> is a "concrete type"
  ...
  private def concretizeClassIfNeeded(
      methodName: String,
      clazz: Z3Type,
      methodTypeArgs: List[Z3Type]
  ): State[CompilerState, Unit] =
    State.modify { state =>
      {
            ...
            case Some(ClassInfo(_, methods, _)) => {
              methods.get(methodName) match {
                case Some(MethodInfo(_, _, concretizer)) => {
                  ...
                  val methodVersions = state.concretizedMethods.getOrElse(
                    className,
                    Map.empty[String, Set[(List[Z3Type], List[Z3Type])]]
                  )
                  val concretizedVersions: Set[(List[Z3Type], List[Z3Type])] =
                    methodVersions.getOrElse(
                      methodName,
                      Set.empty[(List[Z3Type], List[Z3Type])]
                    )
                  ...
                  if (!concretizedVersions.contains(classAndMethodTypeArgs)) {
                    ...
                    val newState = for {
                      _ <- State.modify[CompilerState] {
                        case s => {
                          val extendedConcretizedVersions =
                            concretizedVersions + classAndMethodTypeArgs
                          val extendedMethodVersions =
                            methodVersions + (methodName -> extendedConcretizedVersions)
                          s.copy(concretizedMethods =
                            s.concretizedMethods + (className -> extendedMethodVersions)
                          ) // this state marks the fact that we are concretizing `<className> <..typeArgs>` such that we don't concretize it again (otherwise we end up in an infinite loop)
                        }
                      }
                      concretizedFunction <- concretizer(
                        classTypeArgs,
                        methodTypeArgs
                      )
                      modifiedState <- State.modify[CompilerState](s =>
                        s.copy(reversedProgram =
                          concretizedFunction :: s.reversedProgram
                        )
                      )
                    } yield modifiedState
  ...
  type MethodConcretizer =
    (ConcreteTypeArgs, ConcreteTypeArgs) => State[CompilerState, Function]

  final case class MethodInfo(
      name: String,
      method: Method,
      @transient concretizer: MethodConcretizer
  ) {
  ...
  final case class ClassInfo(
    ...
    def normalize() = {
      copy(methods = methods.map { case (methodName, info) =>
        (z3FunNameToOriginalMethodName(methodName, name), info)
      })
    }

    def keepPublicMethodsOnly(): ClassInfo =
      copy(methods = methods.filter(nameAndMethod => {
        val mods = nameAndMethod._2.method.mods
        !(mods.contains(Private) || mods.contains(Protected))
      }))
  }

  final case class EnumCtorInfo(
      enumName: String,
      fields: List[Var],
      tparams: List[Z3Type]
  ) {
  ...
  /** @param classes
    *   Maps class names to a function that concretizes the class methods using
    *   the provided type parameters.
    * @param concretizedMethods
    *   Maps class name to a map that maps method name to a set of already
    *   concretized versions of that method (e.g. Set -> Map("add" -> Set[Int,
    *   String]))
    * @param enums
    *   Maps data constructor names to information about this data constructor.
    * @param reversedProgram
    *   Compiled program including class definitions, generic methods, and
    *   concretized methods in reversed order.
    */
  final case class CompilerState(
      classes: Map[String, ClassInfo] = Map(),
      concretizedMethods: Map[String, Map[String, Set[
        (List[Z3Type], List[Z3Type])
      ]]] = Map(),
  ...
  def makeFunName(
    ...
    val classNameWithTypeParams = makeTParamName(
      Z3Type(className, classTypeParams)
    )
    val opNameWithTypeParams = makeTParamName(Z3Type(opName, methodTypeParams))
    s"${classNameWithTypeParams}_$opNameWithTypeParams"
  }

  def makeFieldName(fieldName: String, className: String) =
    s"$className>>$fieldName"

  /** Computes the name of a class method based on its Z3 function name. e.g.
    * method `Foo[Int].test` in `class Foo[Int] { def test[A](a: A) = a }` has
    * name `Foo<Int>_test<Foo_test<A>>` in Z3 This function takes the name
    * `Foo<Int>_test<Foo_test<A>>` and the class name `Foo` and returns the
    * original method name `test`.
    */
  def z3FunNameToOriginalMethodName(z3FunName: String, className: String) =
    z3FunName
      .replaceAll(s"$className(\\<.*\\>)?_", "")
      .replaceAll("(\\<.*\\>)?", "")
}

class Z3CompilerPlugin
    extends Plugin[
      Z3CompilerPlugin.Z3Exp,
      Z3CompilerPlugin.MethodOrPredicate,
      Z3CompilerPlugin.CompilerState
    ] {
  ...
  private def makeComplexTypeInternal(
      name: String,
      compiledTypeArgs: List[Z3Exp],
      ogDataCtor: Option[Z3Type] = None
  ) = {
  ...
  def makeComplexType(
      name: String,
      compiledTypeArgs: List[Z3Exp]
  ): State[CompilerState, Z3Exp] = State.inspect {
    case s => {
  ...
  def makeTypeParam(
      name: String,
      tparams: List[Z3Exp],
      upperTypeBound: Option[Z3Exp]
  ): State[CompilerState, Z3Exp] = State.pure(Z3Type(name, Nil))
  ...
  def makeBinaryOperation(
      op: Op.BinaryOperator,
      left: Z3Exp,
      right: Z3Exp
  ): State[CompilerState, Z3Exp] = State.pure {
  ...
  def makeIf(
      ...
      thenb: Z3Exp,
      elseb: Z3Exp
  ): State[CompilerState, Z3Exp] = State.pure(If(condition, thenb, elseb))
      ...
      case tpe => {
        className match {
          case "Set" => {
            ...
            val qualify =
              QualifiedIdentifier("const", Z3Type("Set", List(elemType)))
            Apply(qualify, List("false"))
          }
          case "Map" => {
            val List(keyType, valueType) = tparams.asInstanceOf[List[Z3Type]]
            val qualify = QualifiedIdentifier(
              "const",
              Z3Type("Map", List(keyType, valueType))
            )
            val qualifiedNone =
              QualifiedIdentifier("None", Z3Type("Option", List(valueType)))
  ...
  def makeMethodReference(
    ...
    val paramTypes = tpe.typeArgs.dropRight(
      1
    ) // `tpe.typeArgs` is the type signature: Arg1 Arg2 ... ArgN RetTpe, so we drop the return type
    var ctr = 0
    ...
    for {
      body <- makeMethodCall(
        ...
        params.map(_.name),
        params.map(_.tpe)
      ) // method reference is only allowed to methods that do not have type parameters
  ...
  def makeMethodCall(
    ...
    if (method == "asInstanceOf")
      State.pure(
        obj
      ) // ignore `asInstanceOf` calls, because those are only allowed to cast an enumeration type to a data constructor type, but in Z3 data constructors are of the enumeration type
    else {
      className match {
        case "Set" => {
          ...
          def mapFunction(fun: Z3Exp, forall: Boolean = true) = {
            ...
            Quantifier(
              ...
              List(
                Var("v<>", valueType)
              ), // we use the <> to avoid name clashes with names in user-defined code
              If(contains("v<>"), apply(fun, "v<>"), ifNotPresent)
            )
          }
          ...
          def mapTransformer() = {
            ...
            // v => f(v) == w
            val transformFn =
              Lambda(List(Var(v, valueType)), Equals(apply(f, v), w))

            // lambda (w: W) { set.exists(v => f(v) == w) }
            Lambda(List(Var(w, newValueType)), mapFunction(transformFn, false))
          }
        ...
        case "Map" => {
          ...
          def get(key: Z3Exp) = Get(
            Select(obj, key),
            valueType
          ) // we may be applying `(get None)`, user code should always first check that the key is contained in the map before calling get!
          def mapFunction(fun: Z3Exp, forall: Boolean = true) = {
            /*
            ...
            Quantifier(
              ...
              List(
                Var("k<>", keyType),
                Var("v<>", valueType)
              ), // we use <> to avoid name clashes with user-defined code
              If(
            ...
            method match {
              ...
              case "getOrElse" => {
                ...
                Let(
                  ...
                  If(keyIsPresent, Get("maybeValue", valueType), default)
                )
              }
              case "keys" => {
                /*
                ...
                Lambda(List(Var("key<>", keyType)), contains("key<>"))
              }
              case "values" => {
                /*
                 * m.values() =
                ...
                Lambda(
                  ...
                  Exists(
                    ...
                    Equals(Select(obj, "key<>"), Somee("v<>", valueType))
                  )
                )
              }
              case "bijective" => {
              ...
              case "toSet" => {
                ...
                val key = Apply(
                  QualifiedIdentifier("Tuple>>fst", keyType),
                  List("kvTuple<>")
                )
                val value = Apply(
                  QualifiedIdentifier("Tuple>>snd", valueType),
                  List("kvTuple<>")
                )
                ...
                Lambda(
                  ...
                  Let(
                    List(Var("key<>", key)),
                    And(contains("key<>"), Equals(get("key<>"), value))
                  )
                )
              }
              case "filter" => {
              ...
              case "map" | "mapValues" => {
                ...
                val newValueType =
                  funType.typeArgs.last // return type of the function
                val maybeNewValueType = Z3Type("Option", List(newValueType))

                val maybeValueType = Z3Type("Option", List(valueType))
                val none = QualifiedIdentifier("None", maybeValueType)
                val noneAsNewValueType =
                  QualifiedIdentifier("None", maybeNewValueType)
              ...
              case "zip" => {
                ...
                argTypes.head match {
                  case Z3Type("Map", List(_, value2Type), _) => {
                    ...
                    val noneAsMaybeValue =
                      QualifiedIdentifier("None", maybeValueType)
                    val noneAsMaybeValue2 =
                      QualifiedIdentifier("None", maybeValue2Type)
                    val tupleType = Z3Type("Tuple", List(valueType, value2Type))
                    val maybeTupleType = Z3Type("Option", List(tupleType))
                    val noneAsMaybeTupleType =
                      QualifiedIdentifier("None", maybeTupleType)
                    val qualifiedSomeTupleValue =
                      QualifiedIdentifier("Some", maybeTupleType)
                    ...
                    Lambda(
                      ...
                      Let(
                        ...
                        If(
                          ...
                          Apply(
                            qualifiedSomeTupleValue,
                            List(
                              Apply(
                                makeTuple,
                                List(
                                  Get("maybeValue1<>", valueType),
                                  Get("maybeValue2<>", value2Type)
                                )
                              )
                            )
                          )
              ...
              case "combine" => {
                ...
                val noneAsMaybeValue =
                  QualifiedIdentifier("None", maybeValueType)
                val qualifiedSomeValue =
                  QualifiedIdentifier("Some", maybeValueType)
        ...
        case _ => {
          ...
          concretizeClassIfNeeded(method, clazz, methodTypeArgs) map {
            case _ => {
              ...
              val methodName =
                MethodName(className, classTypeParams, method, methodTypeArgs)
              val extendedArgs =
                if (ProgramCompiler.isProofClass(className)) args
                else
                  obj :: args // methods in proof classes don't take a `this` argument because proof classes don't have fields
  ...
  def makeValDef(
      ...
      tpe: Z3Exp,
      value: Z3Exp
  ): State[CompilerState, Z3Exp] = State.pure(ValDef(name, tpe, value))
  def makeValDecl(name: String, tpe: Z3Exp): State[CompilerState, Z3Exp] =
    State.pure(Z3Wrapper("")) // ignore value declarations
  ...
  def makeBlock(block: List[Z3Exp]): State[CompilerState, Z3Exp] = State.pure {
    ...
    val valDefs = stats
      ...
      .asInstanceOf[List[ValDef]] // keep only value definitions
    val bindings = valDefs.map { case ValDef(name, _, value) =>
      Var(name, value)
    }

    // Transform the value definitions into nested lets
    bindings.reverse.foldLeft(body) { case (body, binding) =>
  ...
  def makeMethodDecl(
      ...
      args: List[Plugin.Arg[Z3Exp]],
      retTpe: Z3Exp
  ): State[CompilerState, Z3Exp] = State.pure(Z3Wrapper(""))

  // A function (i.e. lambda) is an array
  // e.g. (x: A, y: B) => someValueOfTypeC will have type (Array A B C)
  def makeFunctionType(
      paramTypes: List[Z3Exp],
      resType: Z3Exp
  ): State[CompilerState, Z3Exp] =
    makeComplexType("Array", paramTypes.appended(resType))

  def makeFunction(
      params: List[Plugin.Arg[Z3Exp]],
      body: Z3Exp
  ): State[CompilerState, Z3Exp] = State.pure {
  ...
  def makeFunctionCall(
      fun: Z3Exp,
      args: List[Z3Exp]
  ): State[CompilerState, Z3Exp] = State.pure(apply(fun, args: _*))
  ...
  def makeMethodDef(
      ...
      retTpe: Z3Exp,
      isRecursive: Boolean
  ): State[CompilerState, MethodOrPredicate] = {
    State.pure(
      MethodWithoutSpec(
        mods,
        name,
        tparams.asInstanceOf[List[Z3Type]],
        args,
        body,
        retTpe.asInstanceOf[Z3Type],
        isRecursive
      )
    )
  }
  ...
  def makeContextDef(
      ...
      args: List[Plugin.Arg[Z3Exp]],
      body: Z3Exp
  ): State[CompilerState, MethodOrPredicate] = State.pure(Ctx(name, args, body))
  def makePreconditionDef(
      name: String,
      tparams: List[Z3Exp],
      args: List[Plugin.Arg[Z3Exp]],
      body: Z3Exp
  ): State[CompilerState, MethodOrPredicate] = State.pure(Pre(name, args, body))
  def makeInvariantDef(
      name: String,
      tparams: List[Z3Exp],
      args: List[Plugin.Arg[Z3Exp]],
      body: Z3Exp
  ): State[CompilerState, MethodOrPredicate] = State.pure(Inv(name, args, body))

  private def groupMethodsAndPreds(
      xs: List[MethodOrPredicate]
  ): (List[Method], Map[String, Method]) = {
    ...
    val methodDefOrder =
      xs.filter(_.isInstanceOf[MethodWithoutSpec]).map(_.name)
    val groups = SortedMap.from(groupedStats)(
      Ordering.by((s: String) => methodDefOrder.indexOf(s))
    )
    ...
    val methods = groups.values map {
      case methodsAndPreds => {
        methodsAndPreds.find(_.isInstanceOf[MethodWithoutSpec]) match {
          case None =>
            throw new Error(
              s"Missing method definition for ${methodsAndPreds.head.name}"
            )
          case Some(m) => {
            val methodWithoutSpec = m.asInstanceOf[MethodWithoutSpec]
            val ctx = methodsAndPreds
              .find(_.isInstanceOf[Ctx])
              .asInstanceOf[Option[Ctx]]
            val pre = methodsAndPreds
              .find(_.isInstanceOf[Pre])
              .asInstanceOf[Option[Pre]]
            val inv = methodsAndPreds
              .find(_.isInstanceOf[Inv])
              .asInstanceOf[Option[Inv]]
  ...
  def makeClassDef(
      annots: List[String],
      className: String,
      compiledTparams: List[Z3Exp],
      fs: List[Plugin.Field[Z3Exp]],
      mAndPs: List[MethodOrPredicate],
      methodsAndPreds: List[MethodOrPredicate],
      supert: Option[Z3Exp]
  ): State[CompilerState, Z3Exp] = {
    ...
    makeComplexType(className, tparams) flatMap { classType =>
      {
        val fields =
          fs.map(f => f.copy(name = makeFieldName(f.name, className)))
        val fieldLst = fields.map(f => s"(${f.name} ${f.tpe})").mkString(" ")

        def makeMethodTParamName(
            className: String,
            opName: String,
            typeParam: String
        ) = s"${className}_$opName<$typeParam>"
        ...
        def concretizeFilledMethod(
            m: MethodWithFullName,
            concreteTParams: List[Z3Type],
            newClassType: Z3Exp
        ) = m match {
          case MethodWithFullName(
                methodNameObj,
                Method(
                  _,
                  _,
            ...
            val methodArgs =
              if (ProgramCompiler.isProofClass(className)) args
              else self :: args // do not add "this" argument to proofs
            val methodFun =
              makeFunction(methodName, methodArgs, body, retTpe, isRecursive)
        ...
        def concretizeClass(concreteTParams: List[Z3Type]) = {
          ...
          makeComplexType(className, concreteTParams) flatMap { newClassType =>
            {
              val classSubstitutionList = (
                classType.asInstanceOf[Z3Type],
                newClassType.asInstanceOf[Z3Type]
              ) :: (tparams zip concreteTParams)
              ...
              for {
                ...
                ms <- methodsList.traverse(method => {
                  val methodGenericTParamNames = method.tparams.map(tparam =>
                    Z3Type(
                      makeMethodTParamName(className, method.name, tparam.name),
                      Nil
                    )
                  )
                  val methodSubstitutions =
                    method.tparams zip methodGenericTParamNames
                  val methodAndClassSubstitutions =
                    (methodSubstitutions ++ classSubstitutionList)
                      .distinctBy(_._1)
                      .toMap
                  ...
                  for {
                    ...
                    _ <- State.modify[CompilerState] {
                      case s => {
                        ...
                        val methodVersions = s.concretizedMethods.getOrElse(
                          className,
                          Map.empty[String, Set[(List[Z3Type], List[Z3Type])]]
                        )
                        val concretizedVersions
                            : Set[(List[Z3Type], List[Z3Type])] =
                          methodVersions.getOrElse(
                            methodName,
                            Set.empty[(List[Z3Type], List[Z3Type])]
                          )
                        val classAndMethodTypeArgs =
                          (concreteTParams, methodGenericTParamNames)
                    ...
                    filledMethod <- method.fillTypeParameters(
                      methodAndClassSubstitutions
                    )
                  } yield filledMethod
                }) map { newMethods =>
                  {
                    // Now for each of those methods, make a Z3 function
                    val newMethodsWithFullName = newMethods
                      .asInstanceOf[List[Method]]
                      .map(m => m.setFullName(className, concreteTParams))
                    val compiledMethods = newMethodsWithFullName.map(
                      concretizeFilledMethod(_, concreteTParams, newClassType)
                    )
        ...
        def concretizeMethod(
            methodName: String,
            concreteClassTParams: List[Z3Type],
            concreteMethodTParams: List[Z3Type]
        ): State[CompilerState, Z3Code] = {
          // Substitute the given types and also substitute this class type by the concrete class type, e.g. Set<A> to Set<Int>
            ...
            newClassType =>
              {
                methods.get(methodName) match {
                  case Some(method) => {
                    ...
                    val allTParams =
                      (methodTParams zip concreteMethodTParams) ++ (classTParams zip concreteClassTParams)
                    ...
                    method
                      ...
                      .map(
                        _.asInstanceOf[Method]
                          .setFullName(className, concreteClassTParams)
                      )
                      .map(m =>
                        concretizeFilledMethod(
                          m,
                          concreteClassTParams,
                          newClassType
                        )
                      )
                  }
                  case None =>
                    throw new InternalError(
                      s"Method $methodName does not exist in class $className"
                    )
                }
              }
          }
        ...
        val concreteTParamsType =
          tparams.map(tparam => Z3Type(className, List(tparam)))
        ...
        val classTParamDeclarations = concreteTParams
          .map(tparam => s"(declare-sort $tparam)")
          .mkString("\n")
        val classTParamsTypes = concreteTParams.map(
          Z3Type(_, Nil)
        ) // these will be the generic types, e.g. Map<K> instead of (Map K)

        // Also declare a sort for every type parameter of the method
        val methodConcreteTParams = methodsList
          .map(m =>
            m.tparams.map(tparam =>
              makeMethodTParamName(className, m.name, tparam.name)
            )
          )
          .flatten
        val methodTParamDeclarations = methodConcreteTParams
          .map(tparam => s"(declare-sort $tparam)")
          .mkString("\n")
        ...
        for {
          ...
          _ <- State.modify[CompilerState](s =>
            s.copy(reversedProgram =
              methodTParamDeclarations :: classTParamDeclarations :: s.reversedProgram
            )
          )
          ...
          _ <- State.modify[CompilerState] {
            case s => {
              val datatypeDef =
                s"(declare-datatypes (${tparams.mkString(" ")}) (($className ($ctor $fieldLst))))"
              val classInfo = ClassInfo(
                ...
                methodsList
                  .map(m =>
                    (
                      m.name,
                      MethodInfo(m.name, m, concretizeMethod(m.name, _, _))
                    )
                  )
                  .toMap,
                datatypeDef
              ...
              s.copy(
                classes = s.classes + (className -> classInfo),
                reversedProgram = datatypeDef :: s.reversedProgram
              )
            }
          }
          ...
          res <- State[CompilerState, Z3Exp] { s =>
            {
              val genericMethodsStr = genericMethods._1.mkString("\n\n")
              val compiledClass =
                if (!ProgramCompiler.isProofClass(className))
                  ind"""(declare-datatypes (${tparams.mkString(
                        " "
                      )}) (($className ($ctor $fieldLst))))
                  ...
                  .map(m =>
                    (
                      ...
                      MethodInfo(
                        ...
                        m.method,
                        concretizeMethod(m.method.name, _, _)
                      )
                    )
                  )
                  .toMap,
              ...
              val newState = s.copy(
                classes = s.classes + (className -> classInfo),
                reversedProgram = genericMethodsStr :: s.reversedProgram
  ...
  def makeEnumDef(
      enumName: String,
      tparams: List[Z3Exp],
      ctors: List[Plugin.EnumCtor[Z3Exp]]
  ): State[CompilerState, Z3Exp] = {
      ...
      .map {
        case Plugin.EnumCtor(ctorName, _, fields) => {
          if (fields.isEmpty)
            ...
            val fs =
              fields.map(f => f.copy(name = makeFieldName(f.name, ctorName)))
    ...
    State[CompilerState, Z3Exp] { s =>
      {
        ...
        (
          s.copy(
            enums = extendedEnums,
            reversedProgram = enumDef :: s.reversedProgram
          ),
          enumDef
        )
      }
  ...
  private def getConcreteDataCtor(
      ctorTpe: Z3Type
  ): State[CompilerState, EnumCtorInfo] = for {
  ...
  private def compileCase(
      exp: Z3Exp,
      casee: Plugin.Case[Z3Exp]
  ): State[CompilerState, Z3Exp] = casee match {
    // case name: Type => body
    case Plugin.Case(Plugin.Pat.Typed(Plugin.Pat.Var(name), tpe), body) => {
      ...
      for {
        ...
        CtorCase(ctorTpe.name, vars),
        Let(List(Var(name, exp)), body)
      )
    }
    // case _: Type => body
    ...
    // case _ => body
    case Plugin.Case(Plugin.Pat.Wildcard, body) =>
      State.pure {
        // (_ body)
        Pattern(VariableCase("_"), body)
      }
    case Plugin.Case(Plugin.Pat.Var(name), body) =>
      State.pure {
        // (name body)
        Pattern(VariableCase(name), body)
      }
    // case Ctor(var1 ... var_n) => body
    case casee @ Plugin.Case(Plugin.Pat.Extract(tpe, vars), body) =>
      State.pure {
  ...
  /** Compiles a pattern match expression into nested if tests. For example,
    * assume we have an enumeration type E with 3 constructors of type C1, C2,
    * and C3:
    * {{{
    * enum E {
    ...
    * }}}
    *
    * Now, we can pattern match on expressions of type `E`. For example, if
    * `exp` is of type `E`, we can pattern match as follows:
    *
    * {{{
    * exp match {
    *   case i1: C1 => b1
    ...
    * }}}
    */
  def makePatternMatch(
      exp: Z3Exp,
      cases: List[Plugin.Case[Z3Exp]]
  ): State[CompilerState, Z3Exp] = for {
  ...
  def startFile(path: Option[String]): State[CompilerState, Z3Exp] =
    State.pure("")
  def endFile(): State[CompilerState, Z3Exp] = State.pure("")

  def makeLogicVar(name: String, tpe: Z3Exp): State[CompilerState, Z3Exp] =
    State.pure(Var(name, tpe))

  override def makeImplication(
      antecedent: Z3Exp,
      consequent: Z3Exp
  ): State[CompilerState, Z3Exp] = State.pure {
    Apply("=>", List(antecedent, consequent))
  ...
  def run(
      compiler: State[CompilerState, List[Z3Exp]],
      initialState: Option[CompilerState]

...
class ClassTest extends FlatSpec {
  ...
  it should "support modulo on integers" in {
    ...
    val scalaCode = ProgramCompiler(moduloProgram).compileToLanguage(new ScalaCompilerPlugin)._2
    val z3Code = ProgramCompiler(moduloProgram).compileToLanguage(new Z3CompilerPlugin)._2
